...
export const useSettingsStore = defineStore("settings", () => {
  ...
  const defaultSellingPriceList = computed(
    () => sellingSettings.value.default_selling_price_list
  );

  const defaultBuyingPriceList = computed(
    () => buyingSettings.value.default_buying_price_list
  );

  const defaultCurrency = computed(
    () => globalDefaults.value.default_currency
  );

  const defaultCompany = computed(
    () => globalDefaults.value.default_company
  );
  ...
  const allowNegativeStock = computed(
    () => !!erpStockSettings.value.allow_negative_stock
  );

  const valuationMethod = computed(
    () => erpStockSettings.value.valuation_method
  );

  const showBarcodeField = computed(
    () => !!erpStockSettings.value.show_barcode_field
  );

  const validateSellingPrice = computed(
    () => !!sellingSettings.value.validate_selling_price
  );

  const defaultCustomerGroup = computed(
    () => sellingSettings.value.customer_group
  );

  const defaultTerritory = computed(
    () => sellingSettings.value.territory
  );

  const disableRoundedTotal = computed(
    () => !!globalDefaults.value.disable_rounded_total
  );

  const disableInWords = computed(
    () => !!globalDefaults.value.disable_in_words
  );
  ...
  const enableDiscountAccounting = computed(
    () => !!accountsSettings.value.enable_discount_accounting
  );

  const overBillingAllowance = computed(
    () => accountsSettings.value.over_billing_allowance
  );

  const maintainSameRate = computed(
    () => !!buyingSettings.value.maintain_same_rate
  );
