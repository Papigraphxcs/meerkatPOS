<template>
  <div class="flex flex-wrap items-center gap-2">
    ...
    <Select v-model="localFilters.orderBy" @update:model-value="emitFilters">
      ...
      <SelectContentStyled>
        <SelectItemStyled value="posting_date desc, posting_time desc">{{ __("Date (Newest)") }}</SelectItemStyled>
        <SelectItemStyled value="posting_date asc, posting_time asc">{{ __("Date (Oldest)") }}</SelectItemStyled>
      ...
      <PopoverContentStyled class="w-[500px] p-0 overflow-visible" align="end">
        ...
        <div class="p-3 space-y-2 max-h-[300px] overflow-y-auto">
          <div v-for="(filter, index) in queryFilters" :key="index" class="flex items-center gap-2">
            <Autocomplete v-model="filter.field" :options="filterFieldOptions" :placeholder="__('Search fields...')"
              :show-search-icon="false" :clearable="false" :max-visible="10" :empty-text="__('No fields found')"
            ...
            <Input v-if="!['is', 'is not'].includes(filter.operator)" v-model="filter.value" type="text"
              :placeholder="__('Value')" class="h-8 flex-1" @change="emitFilters" />
            <Select v-else v-model="filter.value" @update:model-value="emitFilters">
              ...
              <SelectContentStyled>
                ...
                <SelectItemStyled value="not set">{{ __('Not Set (empty)') }}</SelectItemStyled>
              </SelectContentStyled>
            </Select>
