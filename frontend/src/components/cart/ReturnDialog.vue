            ...
            <div class="flex-1 overflow-y-auto p-5 space-y-4 xpos-scrollbar">
                <div v-if="!selectedInvoice">
                    ...
                    <div v-else-if="invoices.length > 0" class="space-y-2 max-h-60 overflow-y-auto xpos-scrollbar">
                        <button v-for="inv in invoices" :key="inv.name" @click="selectInvoice(inv.name)"
                            ...
                            <span class="font-bold text-foreground text-sm">{{ formatPrice(inv.grand_total) }}</span>
                        </button>
                    </div>
                ...
                <div v-else>
                    <div class="flex items-center justify-between mb-3">
                        <div>
                            <h3 class="text-sm font-semibold text-foreground">{{ selectedInvoice.name }}</h3>
                            <p class="text-xs text-muted-foreground">{{ selectedInvoice.customer_name }}</p>
                        </div>
                    ...
                    <div class="border border-border rounded-lg overflow-hidden">
                        <table class="w-full text-sm">
                            ...
                            <tbody>
                                <tr v-for="(item, idx) in returnItems" :key="idx" class="border-t border-border">
                                    <td class="px-3 py-2">
                                        <p class="font-medium text-foreground">{{ item.item_name }}</p>
                                        <p class="text-[11px] text-muted-foreground font-mono">{{ item.item_code }}</p>
                                    </td>
...
const hasReturnItems = computed(() =>
    returnItems.value.some((i) => i.return_qty > 0)
);
