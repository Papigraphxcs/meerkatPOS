    ...
    <Card class="w-full max-w-lg">
      <template v-if="step === 1">
        ...
        <CardContent class="space-y-3">
          <button @click="config.role = 'hub'" :class="[
          ...
          <button @click="config.role = 'till'" :class="[
            'w-full p-4 rounded-lg border-2 text-start transition-all',
            config.role === 'till'
              ? 'border-primary bg-primary/5'
              : 'border-border hover:border-primary/50'
      ...
      <template v-if="step === 2">
        ...
        <CardContent>
          <div v-if="mariaDbStatus !== null" class="mb-4 p-3 rounded-lg border"
            :class="mariaDbStatus.installed ? 'bg-green-50 dark:bg-green-950/20 border-green-200 dark:border-green-800' : 'bg-amber-50 dark:bg-amber-950/20 border-amber-200 dark:border-amber-800'">
            <div class="flex items-start gap-2">
              <CircleCheck v-if="mariaDbStatus.installed"
              ...
              <div class="text-sm">
                ...
                <template v-else>
                  ...
                  <Button size="sm" variant="ghost" class="mt-2 text-amber-700 dark:text-amber-400"
                    @click="detectMariaDb">
        ...
        <CardContent class="space-y-3">
          <template v-if="config.role === 'hub'">
            ...
            <div>
              ...
              <span v-if="erpTestResult !== null"
                :class="['text-sm ms-2', erpTestResult ? 'text-green-600' : 'text-destructive']">
          ...
          <template v-else>
            <div>
              ...
              <p class="text-xs text-muted-foreground mt-1">{{ __('IP address and port of the hub server on your local network') }}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <span v-if="hubTestResult !== null"
                :class="['text-sm ms-2', hubTestResult ? 'text-green-600' : 'text-destructive']">
      ...
      <template v-if="step === 4">
        ...
        <CardContent class="space-y-4">
          <div class="p-3 rounded-lg bg-primary/5 border border-primary/20 text-sm text-muted-foreground">
            {{ __('This account is stored locally and used to log in to X POS. It is independent of your ERPNext users.') }}<br />
            {{ __('Make sure to remember these credentials as they cannot be recovered. You can create additional users later from the settings.') }}<br />
          ...
          <div class="space-y-3">
            ...
            <div>
              ...
              <div class="relative mt-1">
                <Input v-model="adminUser.password" :type="showAdminPassword ? 'text' : 'password'" placeholder="••••••"
                  class="pe-10" autocomplete="new-password" />
                <button type="button"
              ...
              <p v-if="adminUser.password && adminUser.password.length < 6" class="text-xs text-destructive mt-1">{{
                __('Minimum 6 characters') }}</p>
            </div>
            <div>
              <label class="text-sm font-medium text-foreground">{{ __('Confirm Password') }}</label>
              <Input v-model="adminUser.confirmPassword" :type="showAdminConfirm ? 'text' : 'password'"
                placeholder="••••••" class="mt-1" autocomplete="new-password" />
              <p v-if="adminUser.confirmPassword && adminUser.password !== adminUser.confirmPassword"
                class="text-xs text-destructive mt-1">{{ __('Passwords do not match') }}</p>
            </div>
          </div>
        </CardContent>
        <CardFooter class="justify-between">
        ...
        <CardContent>
          <div class="space-y-3 text-sm">
            ...
            <div class="flex items-center justify-between p-3 rounded-lg bg-muted">
              ...
              <span class="text-foreground font-medium">{{ config.dbUser }}@{{ config.dbHost }}:{{ config.dbPort }}/{{
                config.dbName }}</span>
            </div>
