...
<template>
	<div class="h-full min-h-0 flex flex-col bg-background overflow-hidden">
		<header class="bg-card border-b border-border px-4 py-3 flex items-center justify-between shrink-0">
			...
			<div class="flex items-center gap-4">
				...
				<div v-if="purchaseStore.hasPendingPurchases" class="flex items-center gap-2">
					...
					<Button
						v-if="isOnline()"
						@click="purchaseStore.syncPendingPurchases()"
						variant="outline"
						size="sm"
					...
					>
						<RefreshCw
							class="w-4 h-4 me-1"
							:class="{ 'animate-spin': purchaseStore.isSyncing }"
						/>
						{{ __("Sync") }}
					</Button>
				</div>
				...
				<div
					v-if="purchaseStore.selectedSupplier"
		...
		<div v-if="activeTab !== 'receive'" class="bg-card border-b border-border shrink-0">
			<button
				@click="poHeaderExpanded = !poHeaderExpanded"
			...
			<div v-show="poHeaderExpanded" class="px-4 pb-3 grid grid-cols-6 gap-3">
				<div>
					...
					<Input
						v-model="purchaseStore.poAliasName"
						class="h-8 text-sm"
				...
				<div>
					...
					<Input
						v-model="purchaseStore.poDepartment"
						class="h-8 text-sm"
				...
				<div>
					...
					<Input
						v-model="purchaseStore.poRemarks"
						class="h-8 text-sm"
		...
		<div class="flex-1 flex min-h-0 overflow-hidden">
			<template v-if="activeTab !== 'receive'">
				<div
					class="w-80 border-e border-border bg-card flex flex-col min-h-0 overflow-hidden shrink-0"
				>
