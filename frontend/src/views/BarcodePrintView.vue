                ...
                <div class="p-4 pb-2 space-y-3">
                    ...
                    <div class="flex items-center gap-4">
                        <label class="flex items-center gap-2 text-xs cursor-pointer">
                            <Checkbox :checked="showPrice" @update:checked="showPrice = Boolean($event)" class="rounded" />
                            <span class="select-none">{{ __('Show Price') }}</span>
                        </label>
                        <label class="flex items-center gap-2 text-xs cursor-pointer">
                            <Checkbox :checked="includeQR" @update:checked="includeQR = Boolean($event)" class="rounded" />
                    ...
                    <div v-else>
                        <div class="mb-3 flex items-center justify-between">
                            <span class="text-sm font-medium text-foreground">
                                {{ items.length }} {{ __('item(s)') }} &bull;
                                {{ totalLabels }} {{ __('label(s)') }}
                            </span>
                        </div>
                                ...
                                <thead class="bg-muted/50">
                                    <tr>
                                        ...
                                        <th class="text-center px-3 py-2 font-medium text-muted-foreground w-28">{{
                                            __('Barcode') }}</th>
                                        <th class="text-center px-3 py-2 font-medium text-muted-foreground w-24">{{
                                            __('Qty') }}</th>
                                        <th class="text-center px-3 py-2 font-medium text-muted-foreground w-16"></th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr v-for="(item, idx) in items" :key="idx"
                                        ...
                                        <td class="px-3 py-2 text-center">
                                            <span class="font-mono text-xs">{{ item.barcode || item.item_code }}</span>
                                        </td>
                                        <td class="px-3 py-2">
                                            <div class="flex items-center justify-center gap-1">
                                                ...
                                                <NumberInput v-model="item.qty" :min="1"
...
import {
    Search, Plus, Minus, X, Printer, Trash2, Barcode
...
const labelDims = computed(() => {
    switch (labelSize.value) {
        case "small": return { w: "38mm", h: "25mm", bw: "34mm", bh: "10mm" };
        case "large": return { w: "60mm", h: "40mm", bw: "54mm", bh: "16mm" };
        default: return { w: "50mm", h: "30mm", bw: "44mm", bh: "12mm" };
    }
});
...
async function printLabels() {
    ...
    try {
        ...
        const labels = await call<LabelData[]>(
            "xpos.x_pos.api.barcode_generator.get_item_barcode_labels",
            {
        ...
        for (const label of labels) {
            ...
            if (label.barcode_uri) {
                html += `<div class="label-barcode"><img src="${label.barcode_uri}" alt="${escapeHtml(label.barcode_value)}" /></div>`;
