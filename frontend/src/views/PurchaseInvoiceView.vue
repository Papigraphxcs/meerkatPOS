...
import { Search, Plus, Package, Save, X, Receipt, ScanBarcode, Loader2, Calculator } from "lucide-vue-next";
		...
		<div class="bg-card border-b border-border px-4 py-3 shrink-0">
			<div class="grid grid-cols-6 gap-3">
				<div>
					...
					<Input
						v-model="invoiceHeader.invoice_no"
			...
			<div class="grid grid-cols-6 gap-3 mt-3">
				...
				<div>
					...
					<select
						v-model="invoiceHeader.print_bal"
						class="w-full h-8 px-2 border border-border rounded-md text-sm bg-background"
					>
				...
				<div>
					...
					<select
						v-model="invoiceHeader.inv_size"
						class="w-full h-8 px-2 border border-border rounded-md text-sm bg-background"
					>
				...
				<div class="border-t border-border bg-muted shrink-0">
					<div class="px-4 py-2 border-b border-border grid grid-cols-6 gap-4 text-sm">
						...
						<div class="flex items-center gap-2">
							...
							<NumberInput
								v-model="invoiceSettings.stock_discount_percent"
								:min="0"
								:max="100"
								:precision="2"
						...
						<div class="flex items-center gap-2">
							...
							<NumberInput
								v-model="invoiceSettings.flat_discount"
								:min="0"
								:precision="2"
						...
						<div class="flex items-center gap-2">
							...
							<NumberInput
								v-model="invoiceSettings.misc_charges"
								:min="0"
								:precision="2"
								class="h-7 text-xs w-20"
							/>
						...
						<div class="flex items-center gap-2">
							...
							<NumberInput
								v-model="invoiceSettings.invoice_gst_percent"
								:min="0"
								:max="100"
								:precision="2"
								class="h-7 text-xs w-16"
					...
					<div class="px-4 py-3 flex items-center justify-between">
						<div class="flex items-center gap-6 text-sm">
							<span class="text-muted-foreground"
								>{{ __("Items") }}: <strong>{{ invoiceItems.length }}</strong></span
							>
							<span class="text-muted-foreground"
								>{{ __("Subtotal") }}: <strong>{{ formatCurrency(subtotal) }}</strong></span
							>
							<span class="text-muted-foreground"
								>{{ __("GST") }}: <strong>{{ formatCurrency(totalGst) }}</strong></span
							>
						</div>
