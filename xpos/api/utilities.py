...
Provides the same utility features as POS Awesome:
- Selling price lists
- App info & version
- Sales person names
...
def get_selling_price_lists():
	...
	Same as POS Awesome's get_selling_price_lists.
	"""
...
def get_app_info():
	...
	Same as POS Awesome's get_app_info.
	"""
	...
	return {
		"apps": apps,
		"build_version": get_build_version() if hasattr(get_build_version, "__call__") else "0.0.1",
	}
...
def get_pos_profile_tax_inclusive(pos_profile):
	...
	Same as POS Awesome's get_pos_profile_tax_inclusive.
	"""
	try:
		return cint(frappe.db.get_value("POS Profile", pos_profile, "posa_tax_inclusive"))
	except Exception:
		return 0
...
def get_active_pos_profile(user=None):
	...
	Same as POS Awesome's get_active_pos_profile.
	"""
