...
def get_item_variants(pos_profile, parent_item_code, price_list=None, customer=None):
	...
	variants = frappe.get_all(
		...
		fields=[
			...
			"image", "item_group", "has_batch_no", "has_serial_no",
			"is_stock_item", "brand", "max_discount",
		],
...
def get_stock_availability(items):
	...
	for d in items:
		...
		if batch_no:
			...
			results[f"{item_code}|{warehouse}|{batch_no}"] = flt(get_batch_qty(batch_no, warehouse))
		else:
...
def _parse_scale_barcode(barcode, pos_profile):
	...
	try:
		settings = frappe.get_all(
			...
			fields=["barcode_prefix", "item_code_start", "item_code_end",
					 "qty_start", "qty_end", "qty_decimals"],
			limit=1,
		)
		if not settings:
			return None
