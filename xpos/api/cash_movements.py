...
Provides the same cash movement features as POS Awesome:
...
def get_cash_movement_context(pos_profile, pos_opening_shift):
	...
	# Check if cash movement is enabled
	...
	# Get allowed source/target accounts
	source_accounts = []
	try:
	...
	# Get cash mode of payment account
	cash_account = None
	...
	return {
		...
		"company": pos.company,
		"cost_center": pos.get("cost_center") or frappe.db.get_value(
			"Company", pos.company, "cost_center"
		),
	}
...
def get_shift_cash_movements(
	...
	Same as POS Awesome's get_shift_cash_movements.
	"""
	...
	try:
		movements = frappe.get_list(
			...
			filters=filters,
			fields=[
				"name", "movement_type", "amount", "remarks",
...
def _create_cash_movement_record(
	pos_opening_shift, movement_type, amount, remarks,
	journal_entry, account, company,
):
