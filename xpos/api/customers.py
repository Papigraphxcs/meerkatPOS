    ...
    if loyalty_program_name:
        try:
            ...
            loyalty_program = {
                ...
                "loyalty_points": (
                    lp_details.get("loyalty_points", 0) if lp_details else 0
                ),
                "conversion_factor": (
                    lp_details.get("conversion_factor", 0) if lp_details else 0
                ),
            }
        except Exception:
            loyalty_program = {
...
def register_customer_loyalty(customer, loyalty_program):
    ...
    if cust_doc.loyalty_program:
        if cust_doc.loyalty_program == loyalty_program:
            frappe.throw(
                _("Customer is already enrolled in {0}").format(loyalty_program)
            )
        else:
            frappe.throw(
                _(
