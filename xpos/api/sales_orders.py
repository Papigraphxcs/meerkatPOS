...
Provides the same SO/Quotation features as POS Awesome:
...
def search_orders(company, currency=None, order_name=None):
	...
	Same as POS Awesome's search_orders.
	"""
	filters = {
		"company": company,
		"docstatus": 1,
...
def create_sales_order(data):
	...
	Same as POS Awesome's update_sales_order.
	"""
...
def submit_sales_order(data):
	...
	Same as POS Awesome's submit_sales_order.
	"""
	if isinstance(data, str):
		data = json.loads(data)
...
def create_sales_invoice_from_order(sales_order):
	...
	Same as POS Awesome's create_sales_invoice_from_order.
	"""
...
def create_quotation(data):
	...
	Same as POS Awesome's update_quotation.
	"""
	if isinstance(data, str):
		data = json.loads(data)
...
def submit_quotation(data):
	...
	Same as POS Awesome's submit_quotation.
	"""
	if isinstance(data, str):
		data = json.loads(data)
