...
class XPOSCoupon(Document):
	...
	def validate(self):
		...
		pos_offer = frappe.get_doc("XPOS Offer", self.pos_offer)
	...
	def create_coupon_from_referral(self):
		...
		if ref_doc.primary_offer:
			doc = frappe.new_doc("XPOS Coupon")
...
def check_coupon_code(coupon_code, customer=None, company=None):
	res = {"coupon": None}
	if not frappe.db.exists("XPOS Coupon", {"coupon_code": coupon_code.upper()}):
		res["msg"] = _("Sorry, this coupon code not exists")
		return res

	coupon = frappe.get_doc("XPOS Coupon", {"coupon_code": coupon_code.upper()})
	pos_offer = frappe.get_doc("XPOS Offer", coupon.pos_offer)
...
def update_coupon_code_count(coupon_name, transaction_type):
	coupon = frappe.get_doc("XPOS Coupon", coupon_name)
	if coupon:
