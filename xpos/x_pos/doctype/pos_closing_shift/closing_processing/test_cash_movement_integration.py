...
class TestClosingShiftCashMovementIntegration(unittest.TestCase):
    @patch("xpos.x_pos.doctype.pos_closing_shift.closing_processing.creation.get_payments_entries")
    @patch("xpos.x_pos.doctype.pos_closing_shift.closing_processing.creation.get_pos_invoices")
    @patch("xpos.x_pos.doctype.pos_closing_shift.closing_processing.creation.submit_printed_invoices")
    @patch("xpos.x_pos.doctype.pos_closing_shift.closing_processing.creation.frappe")
    ...
    @patch("xpos.x_pos.doctype.pos_closing_shift.closing_processing.overview.get_payments_entries")
    @patch("xpos.x_pos.doctype.pos_closing_shift.closing_processing.overview.get_pos_invoices")
    @patch("xpos.x_pos.doctype.pos_closing_shift.closing_processing.overview.frappe")
    def test_overview_includes_cash_movements_and_adjusts_cash_expected(
        ...
        mock_frappe.db.get_value.side_effect = lambda dt, name, field: (
            0 if field == "custom_create_pos_invoice_instead_of_sales_invoice" else "Cash"
        )
