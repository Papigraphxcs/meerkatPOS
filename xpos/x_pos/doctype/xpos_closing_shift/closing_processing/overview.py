...
def get_closing_shift_overview(pos_opening_shift):
    ...
    elif getattr(payload, "doctype", None) == "XPOS Opening Shift":
    ...
    if not opening_shift_doc:
        ...
        opening_shift_doc = frappe.get_doc("XPOS Opening Shift", opening_shift_name)

    if opening_shift_doc.doctype != "XPOS Opening Shift":
    ...
    def resolve_shift_link_field(doctype_name):
        ...
        if meta:
            for df in meta.get("fields", []):
                if df.fieldtype == "Link" and df.options == "XPOS Opening Shift":
